...
//#if defined(OUTPUT_TO_CSV)
    std::map<ALGORITHM, std::string> filenames = {
        {ALGORITHM::ROTATE, "o_rotate_mid8_chunk=4_threads=40.csv"},
        {ALGORITHM::ROTATE_COPY, "o_rotateCopy_mid8_chunk=4_threads=40.csv"},
    };
//#endif
	...
	auto chunk_size = 4; //change it as threads number
	    ...
	    hpx::execution::static_chunk_size scs(chunk_size); 
            ...
            for (int i = 0; i < NUM_ITERATIONS + 5; i++)
            {
                ...
                {
                    case ALGORITHM::ROTATE:
                        hpx::rotate(hpx::execution::par.with(scs), arr.begin(),arr.begin()+ mid, arr.end());
                        break;

                    case ALGORITHM::ROTATE_COPY:
                        hpx::rotate_copy(hpx::execution::par.with(scs), arr.begin(),arr.begin()+ mid, arr.end(), res1.begin());
                        break;
                };
